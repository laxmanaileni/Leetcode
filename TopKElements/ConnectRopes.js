//minHeap
const MinCostConnectRopes = (ropeLengths) => {
  let minHeap = new MinPriorityQueue();
  let result = 0;
  for (let i = 0; i < ropeLengths; i++) {
    minHeap.enqueue(ropeLengths[i]);
  }
  while (minHeap.size() > 1) {
    let temp = minHeap.dequeue().element + minHeap.dequeue().element;
    result += temp;
...
console.log(MinCostConnectRopes([1, 3, 11, 5])); //33
